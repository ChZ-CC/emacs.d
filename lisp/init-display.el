;;; init-display.el --- 自定义展示配置杂项 -*- lexical-binding: t -*-
;;; Commentary:
;;; Code:
...
;; 基本设置
;;(setq-default indicate-buffer-boundaries 'left)     ;; 在窗口边缘上显示一个小箭头指示当前 buffer 的边界
(setq-default delete-by-moving-to-trash t)          ;; 删除文件移动到垃圾箱
(setq-default window-combination-resize t)          ;; 新窗口平均其他左右窗口
(setq-default x-stretch-cursor t)                   ;; 将光标拉伸到字形宽度
(setq-default kill-whole-line t)                    ;; C-k时,同时删除该行
